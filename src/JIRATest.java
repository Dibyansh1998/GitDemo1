...
public class JIRATest {
	...
	public static void main(String[] args) {
		...
		String responce = given().relaxedHTTPSValidation().header("Content-Type", "application/json")
				.body("{\r\n" + "    \"username\":\"Dibyansh.verma25\",\r\n" + "    \"password\":\"Dibyansh@1998\"\r\n"
						+ "}")
				.log().all().filter(session).when().post("/rest/auth/1/session").then().log().all().extract().response()
				.asString();
		System.out.println(responce);

		String expectedMessage = "My Name is Dibyansh";
		// Add comment Code
